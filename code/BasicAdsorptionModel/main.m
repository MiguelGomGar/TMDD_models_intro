...
P_exact = binding_exact(L, R0, KD);

% Model B: Standard Langmuir Approximation (Valid if L >>> R)
P_langmuir = binding_langmuir(L, R0, KD);

%% 3. Visualization
figure('Units', 'normalized', 'Position', [0.1, 0.05, 0.7, 1]);

% Plot Exact Model
plot(L, P_exact, 'LineWidth', 2.5, 'Color', [0 0.447 0.741], ...
    'DisplayName', 'General Quadratic Model');
hold on;

% Plot Langmuir Approximation
plot(L, P_langmuir, 'LineWidth', 2.5, 'DisplayName', 'Langmuir Approximation');
